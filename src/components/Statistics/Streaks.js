...
const Streaks = () => {
  ...
  const monthString = new Date(currentYear, streaksMonth, 1).toLocaleString(
    'en-US',
    {
      month: 'long',
    }
  ...
  const firstWeekdayOfMonth =
    new Date(currentYear, streaksMonth, 1).getDay() - 1;
  ...
  const showPastMonthHandler = () => {
    ...
    if (newMonth >= 0 && currentMonth - newMonth < 3) setStreaksMonth(newMonth);
  };
  const showNextMonthHandler = () => {
    const newMonth = streaksMonth + 1;
    if (newMonth > currentMonth) return;
    setStreaksMonth(newMonth);
  };

  return (
    <Card className={classes.streaks}>
      <h3>Streaks</h3>
      <h4>{`${monthString} ${currentYear}`}</h4>
